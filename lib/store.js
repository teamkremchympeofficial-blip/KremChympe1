...
// ---------------------------------------------------------------- Content --
//
// Editable website copy: static labels/headings/notes (`text`), plus the
// package details shown on the booking page and sent to guides on
// Telegram (`package`). Both the booking page and the Telegram admin bot
// read/write through the functions below, so a single edit shows up in
// both places. Anything not overridden here falls back to the defaults
// below (which mirror what used to be hardcoded in index.html), so a
// fresh deploy with no content.json still looks exactly the same.
...
function getContent() {
  const stored = readJson(CONTENT_FILE, {});
  return {
    text: { ...DEFAULT_CONTENT.text, ...(stored.text || {}) },
    package: { ...DEFAULT_CONTENT.package, ...(stored.package || {}) },
  };
}

async function setTextValue(key, value) {
...
async function setPackageField(field, value) {
  const content = getContent();
  content.package[field] = value;
  await writeJsonAtomic(CONTENT_FILE, content);
  return content;
}
...
async function addService(name) {
  const content = getContent();
  content.package.services.push(name);
  await writeJsonAtomic(CONTENT_FILE, content);
  return content;
}

// index is 1-based (matches how the list is numbered when shown to the
// admin over Telegram, so there's no off-by-one for them to trip on).
async function removeService(index) {
  const content = getContent();
  const i = index - 1;
  if (i < 0 || i >= content.package.services.length) return null;
  const [removed] = content.package.services.splice(i, 1);
  await writeJsonAtomic(CONTENT_FILE, content);
