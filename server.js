...
const adminBot = require('./lib/admin-bot');
...
// The booking page now lives on a separate static host (e.g. GitHub
// Pages) while this server just handles the API, so browser requests
// from that page are cross-origin and need CORS explicitly allowed.
// ALLOWED_ORIGINS is a comma-separated list, e.g.
//   ALLOWED_ORIGINS=https://yourname.github.io
// The admin dashboard at /admin/dashboard.html is still served by this
// same app (same-origin), so it doesn't need to be in this list.
...
app.use(cors({
  origin(origin, cb) {
    ...
    // dashboard itself) — always allow those.
    if (!origin || ALLOWED_ORIGINS.includes(origin)) return cb(null, true);
    cb(new Error('Not allowed by CORS'));
  },
}));

// Lightweight, unauthenticated endpoint with no real work to do — used
// purely to wake a sleeping Render instance from the booking page before
// the visitor is ready to submit anything.
app.get('/api/health', (req, res) => {
  res.json({ ok: true });
});

// Public, read-only: the booking page fetches this on load to pick up
// any text/price/services edits the super admin has made via Telegram.
// No auth — this is exactly the copy that's already visible on the page.
...
function formatBookingMessage(b) {
  const fallbackServices = store.getContent().package.services;
  const services = (b.services && b.services.length ? b.services : fallbackServices)
    .map((s) => `• ${s}`)
    .join('\n');
...
app.post('/api/telegram/webhook', async (req, res) => {
  ...
  try {
    // Super-admin text commands (/tours, /setchatid, etc.) — this is what
    // replaced the web dashboard. Anyone not in SUPER_ADMIN_CHAT_IDS is
    // silently ignored, so a stranger texting the bot sees no reaction.
    const message = req.body && req.body.message;
    if (message && typeof message.text === 'string') {
      const chatId = message.chat && message.chat.id;
      const reply = await adminBot.handleMessage(chatId, message.text);
      if (reply) {
        // Telegram rejects messages over 4096 chars (e.g. a long /text
        // listing) — split on blank lines so related lines stay together.
        const CHUNK_LIMIT = 3500;
        const parts = reply.split('\n\n');
        let chunk = '';
        const chunks = [];
        for (const part of parts) {
          if (chunk && (chunk.length + part.length + 2) > CHUNK_LIMIT) {
            chunks.push(chunk);
            chunk = part;
          } else {
            chunk = chunk ? `${chunk}\n\n${part}` : part;
        ...
        for (const c of chunks) await telegram.sendMessage(chatId, c);
      }
      return;
    }
